...
func (h *Handler) SendHandler(c *gin.Context) {
	...
	if err := h.dispatch(c, channels, msg); err != nil {
		h.logger.Error("dispatch send request failed", zap.Error(err))
		statusCode := 500
		if de, ok := err.(*dispatchError); ok && de.isBadRequest() {
			statusCode = 400
		}
...
func (h *Handler) dispatch(c *gin.Context, channels []string, baseMsg plugin.Message) error {
	...
	for i, name := range channels {
		...
		go func(idx int, n plugin.Notifier, chName string) {
			...
			for attempt := 0; attempt < attempts; attempt++ {
				if attempt > 0 {
					// 重试前等待，支持 ctx 取消时提前退出。
					select {
					case <-ctx.Done():
						results[idx] = channelResult{channel: chName, err: ctx.Err()}
						return
				...
				lastErr = n.Send(ctx, msg, h.cfg.NotifierConfig(chName))
				if lastErr == nil {
					break
				}
	...
	if len(failed) > 0 {
		alertTargets := successChannels
		// 所有指定渠道都失败时，回退到其他已配置的渠道发送告警。
		if len(alertTargets) == 0 {
			alertTargets = h.fallbackChannels(channels)
		}
		if len(alertTargets) > 0 {
			...
			h.sendAlert(c, alertTargets, alertMsg)
		}
	}
...
func (h *Handler) sendAlert(c *gin.Context, channels []string, msg plugin.Message) {
	var wg sync.WaitGroup
	for _, name := range channels {
		...
		go func(n plugin.Notifier, chName string) {
			defer wg.Done()
			if err := n.Send(c.Request.Context(), msg, h.cfg.NotifierConfig(chName)); err != nil {
